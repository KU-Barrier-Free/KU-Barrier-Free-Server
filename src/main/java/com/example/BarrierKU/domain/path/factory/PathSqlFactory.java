            ...
            FROM (
                SELECT * FROM pgr_dijkstra(
                    ...
                    SELECT id,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER) AS source,
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER) AS target,
                           ((CASE WHEN type = 90 THEN 10000 WHEN degree > 4.8 THEN 100 WHEN degree >= 2.4 THEN 50 ELSE 0 END * 0.5) +
                            (CASE WHEN width < 1 THEN 100 WHEN width < 2 THEN 50 ELSE 0 END * 0.5)) AS cost,
                           ((CASE WHEN type = 90 THEN 10000 WHEN degree > 4.8 THEN 100 WHEN degree >= 2.4 THEN 50 ELSE 0 END * 0.5) +
                            (CASE WHEN width < 1 THEN 100 WHEN width < 2 THEN 50 ELSE 0 END * 0.5)) AS reverse_cost
                    FROM way
                    UNION ALL
                    SELECT id + 100000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
