            ...
            SELECT * FROM pgr_dijkstra(
                ...
                SELECT * FROM (
                    SELECT id,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER) AS source,
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER) AS target,
                           CASE WHEN type = 90 THEN 999999 ELSE length END AS cost,
                           CASE WHEN type = 90 THEN 999999 ELSE length END AS reverse_cost
                    FROM way
                    UNION ALL
                    SELECT id + 100000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER),
                           CASE WHEN type = 90 THEN 999999 ELSE length END,
                           CASE WHEN type = 90 THEN 999999 ELSE length END
                    FROM crossing
                    UNION ALL
                    SELECT id + 200000,
