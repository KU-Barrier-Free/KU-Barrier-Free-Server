...
public class PathSqlFactory {
    ...
    public static String getSql(PathType pathType) {
        return switch (pathType) {
            ...
            case NO_STAIRS -> getNoStairsPathSql();
            ...
            FROM (
                SELECT * FROM pgr_dijkstra(
                    ...
                    SELECT id,
                           ...
                           length AS cost,
                           length AS reverse_cost
                    FROM way WHERE type != 90
                    UNION ALL
                    SELECT id + 100000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER),
                           length, length
                    FROM crossing WHERE type != 90
                    UNION ALL
                    SELECT id + 200000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER),
                           length, length
                    FROM street WHERE type != 90
                    $$,
                    ?, ?, false
                )
            ) r
            ...
            FROM (
                SELECT * FROM pgr_dijkstra(
                    ...
                    SELECT id,
                           ...
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER) AS target,
                           (
                               (CASE WHEN degree > 4.8 THEN NULL WHEN degree >= 2.4 THEN 50 ELSE 100 END * 0.5) +
                               (CASE WHEN width < 1 THEN 0 WHEN width < 2 THEN 50 ELSE 100 END * 0.5)
                           ) AS cost,
                           (
                               (CASE WHEN degree > 4.8 THEN NULL WHEN degree >= 2.4 THEN 50 ELSE 100 END * 0.5) +
                               (CASE WHEN width < 1 THEN 0 WHEN width < 2 THEN 50 ELSE 100 END * 0.5)
                           ) AS reverse_cost
                    FROM way WHERE type != 90 AND degree <= 4.8
                    UNION ALL
                    SELECT id + 100000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER),
                    ...
                    FROM crossing WHERE type != 90 AND degree <= 4.8
                    UNION ALL
                    SELECT id + 200000,
                           CAST(SPLIT_PART(f_node, '#', 2) AS INTEGER),
                           CAST(SPLIT_PART(t_node, '#', 2) AS INTEGER),
                    ...
                    FROM street WHERE type != 90 AND degree <= 4.8
                    $$,
                    ?, ?, false
                )
            ) r
